...
const initDatabase = async () => {
  try {
    ...
    try {
      ...
      console.log('✅ 資料庫連接正常，用戶數量:', result?.count || 0);
    } catch (err) {
      console.error('❌ 用戶表可能不存在，嘗試創建:', err.message);
      ...
      try {
        ...
        expirationDate.setFullYear(expirationDate.getFullYear() + 1); // 1年後到期
        ...
        console.error('❌ 資料庫初始化失敗:', initError);
      }
    }
  } catch (error) {
